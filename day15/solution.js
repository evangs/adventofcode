...
ehom.santa.Ingredient = function Ingredient(name, capacity, durability, flavor, texture, calories) {
    this.name = name;
...
ehom.santa.findBestCookie = function findBestCookie(input, units) {

    var combinations = Combinatorics.baseN([0, 1, 2, 3], units);
    ...
    for (var i = 0; i < combinations.length; i++) {
        ...
        for (var j = 0; j < combinations[i].length; j++) {
            ...
            if (combinations[i][j] === 0) {
                ...
                candy++;
            } else if (combinations[i][j] == 3) {
                chocolate++;
            }
        }

        var tmp = ehom.santa.bestCookie(input, sugar, sprinkles, candy, chocolate);
        if (tmp > highest) {
            highest = tmp;
        }
    }
...
ehom.santa.bestCookie = function bestCookie(input, sugar, sprinkles, candy, chocolate) {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        ingredients[parts[0]] = new self.Ingredient(parts[0], parseFloat(parts[2]), parseFloat(parts[4]), parseFloat(parts[6]), parseFloat(parts[8]), parseFloat(parts[10]));
    }
//    console.log(ingredients);
    var recipe = {};
    recipe.Sugar = sugar; //24
    recipe.Sprinkles = sprinkles; //9
    recipe.Candy = candy; //25
    recipe.Chocolate = chocolate; //42

    var entry;
    var totals = [0,0,0,0];
    for (entry in recipe) {
    //    console.log(entry);
    //    console.log(ingredients[entry]);
    //    console.log(recipe[entry]);
        totals[0] += (recipe[entry] * ingredients[entry].capacity);
        totals[1] += (recipe[entry] * ingredients[entry].durability);
        totals[2] += (recipe[entry] * ingredients[entry].flavor);
        totals[3] += (recipe[entry] * ingredients[entry].texture);
    }
    console.log(totals);
    var score = 1;
